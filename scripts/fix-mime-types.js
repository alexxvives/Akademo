...
async function main() {
  ...
  // Write SQL file
  const sqlPath = path.join(__dirname, '..', 'docs', 'onboarding', 'maximoexponente', 'fix-mime-types.sql');
  fs.writeFileSync(sqlPath, sqlLines.join('\n') + '\n');
  console.log(`\n📝  SQL written to ${sqlPath}`);
  console.log('    Apply with:');
  console.log('    npx wrangler d1 execute akademo-db --remote --file=docs/onboarding/maximoexponente/fix-mime-types.sql');
}
