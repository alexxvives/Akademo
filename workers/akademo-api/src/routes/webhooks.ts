...
webhooks.post('/zoom', async (c) => {
  try {
    ...
    if (event === 'meeting.started') {
      ...
      const participantRegistrantId = participant?.registrant_id || '';
      const isHost = !!participant?.is_host;

      console.log(`[Zoom Webhook] Waiting room participant: uuid=${participantUUID} email="${participantEmail}" user_id="${participantUserId}" registrant_id="${participantRegistrantId}" is_host=${isHost}`);
      ...
      // Meetings now use registration_type:1 — only people with a valid tk= registrant URL
      // can reach this point. Zoom's own registration enforcement is the primary gate.
      // Anyone in the waiting room of a registered meeting has already proven they have a
      // valid URL issued by Akademo — auto-admit them.
      //
      // For legacy meetings (created before registration was enabled) or meetings where
      // waiting_room:true is still set, we fall back to email-based check.
      if (participantEmail) {
        // Email-based fallback (legacy meetings / host email always visible)
        ...
        // No email — participant reached waiting room on a registration-required meeting.
        // They must have a valid tk= URL (Zoom wouldn't let them in otherwise), so admit.
        console.log(`[Zoom Webhook] Waiting room: admitting no-email participant (has valid registrant URL) uuid=${participantUUID} user_id=${participantUserId}`);
        shouldAdmit = true;
      }
