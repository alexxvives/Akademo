  ...
  try {
    ...
    if (event === 'meeting.started') {
      ...
      if (participantRegistrantId) {
        // Primary gate: participant joined via a registrant URL generated by Akademo.
        // Look up by registrant_id — works even when Zoom hides the email (external/Google accounts).
        const enrollment = await c.env.DB
          .prepare(`
            SELECT e.id, e.paymentFrequency, e.nextPaymentDue FROM ClassEnrollment e
            WHERE e.zoomRegistrantId = ? AND e.classId = ? AND e.status = 'APPROVED'
            LIMIT 1
          `)
          .bind(participantRegistrantId, stream.classId)
          .first() as any;
        ...
        if (enrollment) {
          ...
          // registrant_id not in our DB — could be a co-host or teacher added directly in Zoom.
          ...
          if (participantEmail) {
            const teacherOrOwner = await c.env.DB
              ...
              .first();
            shouldAdmit = !!teacherOrOwner;
          }
        }
      } else if (participantEmail) {
        // Fallback for meetings created before registration was enabled (backwards compatibility).
        const teacherOrOwner = await c.env.DB
          .prepare(`SELECT id FROM User WHERE LOWER(email) = ? AND (id = ? OR id = ?) LIMIT 1`)
          .bind(participantEmail, stream.teacherId || '', stream.ownerId || '')
          .first();
        ...
        // No registrant_id and no email — block.
        console.warn(`[Zoom Webhook] Waiting room: blocking participant with no registrant_id and no email uuid=${participantUUID}`);
      }
