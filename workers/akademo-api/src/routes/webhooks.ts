...
// POST /webhooks/daily - Daily.co webhook handler (recording ready → upload to Bunny)
webhooks.post('/daily', async (c) => {
  try {
    ...
    const eventType: string = payload.action || payload.type || '';
    console.log(`[Daily Webhook] Event: "${eventType}"`);

    if (eventType === 'recording.ready-to-download') {
      const recordingId: string = payload.recording_id || payload.properties?.recording_id;
      const roomName: string = payload.room_name || payload.properties?.room_name;

      if (!recordingId || !roomName) {
        console.error('[Daily Webhook] Missing recording_id or room_name', payload);
        return c.json(successResponse({ received: true }));
      }

      // Find the DailyTestRoom by roomName
      const room = await c.env.DB.prepare(
        "SELECT id, roomName FROM DailyTestRoom WHERE roomName = ? AND status != 'ended'"
      ).bind(roomName).first<{ id: string; roomName: string }>();

      if (!room) {
        console.log(`[Daily Webhook] No active room found for roomName: ${roomName}`);
        return c.json(successResponse({ received: true }));
      }

      const apiKey = c.env.DAILY_API_KEY;
      if (!apiKey) {
        console.error('[Daily Webhook] DAILY_API_KEY not set');
        return c.json(successResponse({ received: true }));
      }

      // Mark as processing to prevent duplicate runs
      await c.env.DB.prepare(
        "UPDATE DailyTestRoom SET recordingStatus = 'processing' WHERE id = ?"
      ).bind(room.id).run();

      try {
        // Get fresh download link from Daily.co
        ...
        if (!linkRes.ok) {
          ...
          console.error('[Daily Webhook] Failed to get access link:', errText);
          await c.env.DB.prepare(
        ...
        // Upload to Bunny via their fetch-from-URL endpoint
        const videoTitle = `Daily.co Recording — ${roomName} — ${new Date().toLocaleDateString()}`;
        const bunnyRes = await fetch(
          `https://video.bunnycdn.com/library/${c.env.BUNNY_STREAM_LIBRARY_ID}/videos/fetch`,
          {
            ...
            body: JSON.stringify({ url: downloadUrl, title: videoTitle }),
          }
        );

        if (!bunnyRes.ok) {
          const errText = await bunnyRes.text();
          console.error('[Daily Webhook] Bunny fetch failed:', errText);
          await c.env.DB.prepare(
            "UPDATE DailyTestRoom SET recordingStatus = 'error' WHERE id = ?"
          ).bind(room.id).run();
          return c.json(successResponse({ received: true }));
        ...
        console.log(`[Daily Webhook] Recording uploaded to Bunny: ${videoGuid} for room ${room.id}`);
      } catch (err: any) {
        console.error('[Daily Webhook] Processing error:', err.message);
        await c.env.DB.prepare(
          "UPDATE DailyTestRoom SET recordingStatus = 'error' WHERE id = ?"
        ).bind(room.id).run();
      }
