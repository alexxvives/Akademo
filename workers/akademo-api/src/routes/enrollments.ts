  ...
  try {
    ...
    if (classId) {
      ...
      if (session.role === 'TEACHER') {
        ...
        const classCheck = await c.env.DB
          .prepare('SELECT c.id FROM Class c JOIN Academy a ON c.academyId = a.id WHERE c.id = ? AND a.ownerId = ?')
          .bind(classId, session.id)
          .first();
        if (!classCheck) {
          return c.json(errorResponse('Class not found or not authorized'), 403);
      ...
      const result = await c.env.DB
        .prepare(`
          SELECT 
            ...
            u.suspicionCount as student_suspicionCount,
      ...
      const enrollments = (result.results || []).map((row: any) => ({
        ...
        student: {
          ...
          suspicionCount: (row.student_suspicionCount as number) || 0,
        },
        class: {
          id: row.class_id,
          name: row.class_name,
