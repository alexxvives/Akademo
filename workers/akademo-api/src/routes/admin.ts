...
admin.post('/bulk-import', async (c) => {
  try {
    ...
    if (Array.isArray(urlRows) && urlRows.length > 0) {
      // Group: courseName → sectionName → links[]
      ...
      for (const row of (urlRows as Array<{ linkTitle?: string; url?: string; courseName?: string; sectionName?: string; sectionNumber?: string }>) ) {
        ...
        const bySec = byCourseSec.get(course)!;
        if (!bySec.has(section)) bySec.set(section, []);
        bySec.get(section)!.push({ linkTitle: linkTitle || url, url });
      }
      const urlNow = new Date().toISOString();
      for (const [courseName, bySec] of byCourseSec) {
        const classId = classMap.get(courseName.toLowerCase().trim());
        if (!classId) {
      ...
      log(`url import done: ${linksCreated} links created`);
    }
    ...
    return c.json(successResponse({ created, skipped, errors, total: rows.length, classesCreated, classesUnmatched: unmatchedClassNames.size, classResults, results, quizzesCreated, questionsCreated: questionsCreatedCount, quizResults: quizResultsList, documentsCreated, linksCreated, pdfManifest }));
  } catch (error: any) {
    if (error.message === 'Unauthorized' || error.message === 'Forbidden') throw error;
