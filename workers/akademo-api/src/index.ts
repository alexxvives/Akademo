...
app.get('/teacher/academy', async (c) => {
  try {
    ...
    const result = await c.env.DB.prepare(`
      SELECT a.id, a.name, a.logoUrl, a.paymentStatus, a.requireGrading, a.teachersCanExpel
      FROM Academy a
      JOIN Teacher t ON t.academyId = a.id
      WHERE t.userId = ?
    `).bind(session.id).first<{ id: string; name: string; logoUrl: string | null; paymentStatus: string | null; requireGrading: number | null; teachersCanExpel: number | null }>();
    if (!result) {
