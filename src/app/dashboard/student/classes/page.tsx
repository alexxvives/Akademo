...
export default function StudentClassesPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Live Stream Alert Banner */}
      {activeStreams.length > 0 && (
        <div className="bg-gradient-to-r from-red-500 to-red-600 rounded-xl p-4 shadow-lg border-2 border-red-400">
          <div className="flex items-center gap-4">
            <div className="flex-shrink-0">
              <div className="relative">
                <span className="w-4 h-4 bg-white rounded-full animate-pulse flex items-center justify-center">
            ...
            <div className="flex-1">
              <h3 className="text-white font-bold text-lg flex items-center gap-2">
                ...
                {activeStreams.length === 1 ? '¡Clase en vivo ahora!' : `¡${activeStreams.length} clases en vivo ahora!`}
              </h3>
              <p className="text-white/90 text-sm mt-1">
                {activeStreams.length === 1 
                  ? `${activeStreams[0].className} - ${activeStreams[0].title || 'Stream en vivo'}`
                  : 'Tienes múltiples clases transmitiendo en vivo. Desplázate hacia abajo para unirte.'}
              </p>
            </div>
            <div className="flex-shrink-0">
              <button
                onClick={() => {
                  ...
                  if (firstStream) {
                    const classElement = document.querySelector(`[data-class-id="${firstStream.classId}"]`);
                    if (classElement) {
                      classElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    }
                  }
                }}
                className="px-4 py-2 bg-white text-red-600 rounded-lg font-semibold hover:bg-red-50 transition-colors flex items-center gap-2 shadow-md"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
