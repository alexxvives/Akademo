...
export default function StudentLivePage() {
  ...
  const [joining, setJoining] = useState<string | null>(null); // classId being joined
  ...
  const handleJoin = async (classId: string) => {
    ...
    setJoinError(null);
    try {
      const res = await apiClient(`/live/join?classId=${classId}`);
      const result = await res.json();
      if (result.success && result.data?.joinUrl) {
        window.open(result.data.joinUrl, '_blank', 'noopener,noreferrer');
      } else {
        setJoinError(result.error || 'No se pudo obtener el enlace de acceso');
      }
    } catch {
            ...
            <div key={stream.id} className="bg-white rounded-xl border border-gray-200 p-6">
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => handleJoin(stream.classId)}
                    disabled={joining === stream.classId}
