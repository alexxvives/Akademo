...
export default function TeacherDailyTestPage() {
  ...
  useEffect(() => {
    ...
    apiClient('/auth/me').then(r => r.json()).then(r => {
      if (r.success && r.data) setUserInfo({ name: `${r.data.firstName} ${r.data.lastName}`.trim(), email: r.data.email || '', id: r.data.id || '' });
    }).catch(() => {});
  }, [loadActiveRoom]);
    ...
    <div className="space-y-6">
      ...
      {activeRoom ? (
        <div className="relative rounded-xl overflow-hidden border border-gray-200 bg-black" style={{ height: '70vh', minHeight: 480 }}>
          ...
          />
          {userInfo.name && <DailyWatermark name={userInfo.name} email={userInfo.email} userId={userInfo.id} />}
        </div>
      ) : (
