...
export default function TeacherAssignments() {
  ...
  const handleCreateAssignment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiPost('/assignments', {
        ...
        dueDate: newDueDate || null, maxScore: newMaxScore, uploadId,
      });
      const result = await res.json();
      if (result.success) {
  ...
  const resetForm = () => {
    setNewTitle(''); setNewDescription(''); setNewDueDate(''); setNewMaxScore(100);
            ...
            <form onSubmit={handleCreateAssignment} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="number" value={newMaxScore} onChange={(e) => setNewMaxScore(Number(e.target.value))} min="1"
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500" />
                </div>
              </div>
              <div>
