...
export default function QuizTakingModal({ assignmentId, assignmentTitle, maxScore, alreadyAttempted, onClose, onCompleted }: Props) {
  ...
  // Load questions or results on mount
  useEffect(() => {
    if (alreadyAttempted) {
      loadResult();
    } else {
  ...
  async function loadQuestions() {
    try {
      ...
      if (data.success) {
        ...
        // If already attempted, load the result instead
        if (data.error?.includes('ya has realizado')) {
          await loadResult();
        } else {
  ...
  async function loadResult() {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        // Also load questions for showing alongside results
        const qRes = await apiClient(`/assignments/${assignmentId}/questions`);
        const qData = await qRes.json();
        setResult({ ...data.data, maxScore, questions: qData.success ? qData.data : [] });
      }
    } catch {
      setError('Error al cargar resultado');
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6">
          <div className="text-center mb-6">
            <h2 className="text-2xl font-semibold mb-1">Resultado del Cuestionario</h2>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6">
        ...
        <p className="text-sm text-gray-500 mb-6">{questions.length} preguntas · {maxScore} puntos</p>
