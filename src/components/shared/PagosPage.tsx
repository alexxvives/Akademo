...
export default function PagosPage({ role }: PagosPageProps) {
    ...
    try {
      ...
      if (result.success && result.data) {
        setSelectedStudent({
          ...
          className,
          enrollmentDate,
          paymentData: { ...result.data, classId },
          classId,
        });
      } else {
        setSelectedStudent({
  ...
  const loadData = useCallback(async () => {
    try {
      if (isAdmin) {
        // Admin: fetch academies list and payments across all academies
        ...
        // Build classes list from payments
        ...
        if (academyResult.success && Array.isArray(academyResult.data) && academyResult.data.length > 0) {
          ...
          if (status === 'NOT PAID') {
            ...
            const addToEnrollmentMap = (sId: string, cId: string, cName: string) => {
              if (!enrollmentMap[sId]) enrollmentMap[sId] = [];
              if (!enrollmentMap[sId].some(e => e.classId === cId)) enrollmentMap[sId].push({ classId: cId, className: cName });
            };
            
            demoPending.forEach((p) => {
              const sId = p.studentEmail;
              const cId = p.className === 'Programación Web' ? 'demo-c1' :
                         p.className === 'Matemáticas Avanzadas' ? 'demo-c2' :
                         p.className === 'Diseño Gráfico' ? 'demo-c3' : 'demo-c4';
              addToEnrollmentMap(sId, cId, p.className);
              if (!studentMap.has(sId)) studentMap.set(sId, { id: sId, firstName: p.studentFirstName, lastName: p.studentLastName, email: p.studentEmail });
            });
            
            demoHistory.forEach((p) => {
              const sId = p.studentEmail;
              const cId = p.classId || (p.className === 'Programación Web' ? 'demo-c1' :
                         p.className === 'Matemáticas Avanzadas' ? 'demo-c2' :
                         p.className === 'Diseño Gráfico' ? 'demo-c3' : 'demo-c4');
              addToEnrollmentMap(sId, cId, p.className);
              if (!studentMap.has(sId)) studentMap.set(sId, { id: sId, firstName: p.studentFirstName, lastName: p.studentLastName, email: p.studentEmail });
        ...
        if (studentsResult.success && Array.isArray(studentsResult.data)) {
          ...
          if (pendingResult.success && Array.isArray(pendingResult.data)) {
            pendingResult.data.forEach((p: PendingPayment) => addToEnrollmentMap(p.studentId, p.classId, p.className));
          }
          if (historyResult.success && Array.isArray(historyResult.data)) {
            historyResult.data.forEach((p: PaymentHistory) => addToEnrollmentMap(p.studentId, p.classId, p.className));
          }
          setStudentEnrollments(enrollmentMap);
        }
      }
          ...
          <tbody className="divide-y divide-gray-200">
            ...
            {filteredPaymentHistory.map((history, index) => (
              <tr
                ...
                className={isAcademy ? 'hover:bg-gray-50 cursor-pointer transition-colors' : 'hover:bg-gray-50 transition-colors'}
              >
                <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                ...
                {isAcademy && (
                  <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                    <button
                      ...
                      className="text-sm text-red-600 font-medium flex items-center gap-1.5 hover:text-red-700 disabled:cursor-not-allowed"
        ...
        <StudentPaymentDetailModal
          ...
          onClassChange={async (classId) => {
            if (classId === 'all') {
              ...
              const sc = studentEnrollments[selectedStudent.studentId]?.find(c => c.classId === classId);
              if (sc) {
                await showStudentPaymentHistory(
                  selectedStudent.studentId,
                  selectedStudent.name,
