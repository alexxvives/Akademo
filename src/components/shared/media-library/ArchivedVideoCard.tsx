...
export function ArchivedVideoCard({ video, canDelete, onDelete }: Props) {
  ...
  // Is this video backed by Bunny Stream (archived from a lesson)?
  const bunnyGuid = video.mimeType === 'video/stream' && video.storageKey.startsWith('bunnystream:')
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (bunnyGuid) {
        // Redirected to CDN URL — open in new tab
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden flex flex-col hover:shadow-md transition-shadow group">
      ...
      {/* Blob preview modal (storage videos) */}
      {previewUrl && (
        <div
        ...
        >
          <div className="relative w-full max-w-4xl mx-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="relative w-full" style={{ paddingBottom: '56.25%' }}>
              <iframe
                src={`${getBunnyEmbedUrl(bunnyGuid)}?autoplay=true`}
