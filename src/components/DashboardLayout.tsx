...
export default function DashboardLayout({
  ...
  const handleLogout = useCallback(async () => {
    ...
    // Students: redirect back to their academy join page (login tab)
  ...
  useEffect(() => {
    ...
    if (role === 'STUDENT') {
      // Show suspicion warning if set during login (from join pages)
...
function SuspicionModal({ onClose }: { onClose: () => void }) {
  useEffect(() => {
