...
import {
  ...
  getTooltipStyle,
  type SpotlightRect,
} from './StudentTutorial.constants';
...
export function StudentTutorial() {
  ...
  useEffect(() => {
    ...
    // Fast-path: if we already cached the userId check per-user key
    const cachedUserId = localStorage.getItem('akademo_student_id');
    if (cachedUserId) {
      const key = perUserKey(cachedUserId);
      if (localStorage.getItem(key)) return;
    }

    apiClient('/student/tutorial-status')
      .then((res) => res.json() as Promise<{ success: boolean; data: { seen: boolean; userId: string } }>)
      .then((res) => {
        const userId = res?.data?.userId;
        if (userId) {
          localStorage.setItem('akademo_student_id', userId);
  ...
  const dismiss = useCallback(() => {
    localStorage.setItem(storageKeyRef.current, '1');
    setVisible(false);
    apiClient('/student/tutorial-seen', { method: 'PATCH' as const }).catch(() => {});
  }, []);
